...
bundle=bundle_path.read_text();css=css_path.read_text();runtime=gzip.decompress(base64.b64decode(runtime_path.read_text())).decode().strip();styles=gzip.decompress(base64.b64decode(styles_path.read_text())).decode().strip()
...
# Dedicated V37 presentation sits above V36's content-specific layer and below the long-lived V15 flow layer.
...
# Universal resource dock row, shown only when the fighter actually has Boogie Woogie.
...
# V37 is the last compatibility sweep. It runs after V36.8 has already adapted the historical tests.
