...
assert.equal(manifest.preservation.constitutionHash,'3684c969');
assert.equal(manifest.preservation.baseConstitutionHash,'7598b438');
assert.equal(manifest.preservation.existingAbilitiesPreserved,true);
...
assert.equal(exported.constitutionHash,'3684c969');
...
assert.deepEqual(core.registry.validate(),{ok:true,issues:[],counts:{powers:51,items:210,races:24,traits:48}});
...
assert.equal(coverage.total,225);
assert.equal(coverage.typed,225);
