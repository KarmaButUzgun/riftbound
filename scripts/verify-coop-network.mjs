...
for (const marker of ['HOST RUN','JOIN RUN','EventSource','snapshotRun','sendIntent','window.RIFT_COOP']) assert.ok(client.includes(marker), `client missing ${marker}`);
for (const marker of ['/api/health','/api/rooms','text/event-stream','Only the host can publish authoritative run state','maxPlayers']) assert.ok(server.includes(marker), `server missing ${marker}`);
console.log('LAN Co-op Alpha verification passed.');
