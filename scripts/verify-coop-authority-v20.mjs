...
const child=spawn(process.execPath,['multiplayer/server.mjs'],{cwd:root,env:{...process.env,PORT:String(port),RIFTBOUND_COOP_HOST:'127.0.0.1'},stdio:['ignore','pipe','pipe']});
...
try{
 const health=await waitForServer();assert.equal(health.protocolVersion,2);assert.equal(health.authority,'host');
 ...
 const host=hosted.body,code=host.room.id;assert.equal(host.room.protocolVersion,2);
 ...
 result=await json(`/rooms/${code}/snapshot`,{method:'POST',headers:headers(host),body:JSON.stringify({phase:'combat',turn:1,state:{phase:'combat',turn:1,coop:{protocolVersion:2}}})});assert.equal(result.response.status,200);
 result=await json(`/rooms/${code}/intent`,{method:'POST',headers:headers(partner),body:JSON.stringify({type:'action',sequence:1,payload:{actionId:'strike',targetId:'enemy'}})});assert.equal(result.response.status,202);const intentId=result.body.intentId;
 await delay(110);
 ...
 const state=await json(`/rooms/${code}/state`,{headers:headers(partner)});assert.equal(state.body.room.protocolVersion,2);assert.equal(state.body.snapshot.phase,'combat');
 console.log('Riftbound V20 host-authoritative co-op verification passed.');
}finally{
