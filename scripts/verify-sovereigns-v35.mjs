...
try{
 ...
 assert.ok(unshackled.moves[3].tags.includes('v35Hijack'),'registered Hijack identity drifted');
 ...
 const effectiveHijack=api.hotfix.rewriteActions({power:unshackled,statuses:{}},[{id:'hijack',slot:8,moveIndex:3,name:'Hijack',type:'ultimate',move:unshackled.moves[3]}])[0];assert.ok(effectiveHijack,'effective Hijack action missing');assert.ok(!effectiveHijack.move.tags.includes('selfCast'),'effective Hijack action must not remain self/global cast');assert.ok(effectiveHijack.move.tags.includes('target'),'effective Hijack action must target an enemy');
 ...
 const constitution=preservation.assert();assert.ok(constitution.ok,'V35 constitution assertion failed');assert.equal(constitution.baseHash,'7598b438');assert.equal(constitution.basePreserved,true,'protected V20 foundation changed');assert.deepEqual(constitution.counts,{abilities:61,moves:237});assert.equal(constitution.expected,test.RIFT_V35_EXPECTED_HASH);assert.deepEqual(constitution.addedPowers,['Ruined King','The Unshackled','Ragegod']);
