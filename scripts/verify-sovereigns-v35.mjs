...
try{
 ...
 const bork=test.RIFT_ITEM('blade-ruined-king');assert.ok(bork,'Blade of The Ruined King item missing');assert.equal(bork.rarity,'Legendary');assert.equal(bork.category,'Weapon');assert.match(bork.passive,/9%/);assert.match(bork.passive,/10%/);assert.match(bork.passive,/Clawing Shadows/i);assert.match(bork.passive,/30%/);
